...
class SimpleSentenceEmbeddingModule(nn.Module):
    ...
    def forward(self, query_labels, query_masks):
        ...
        pack_wemb = nn.utils.rnn.pack_padded_sequence(wemb, length.to('cpu'), batch_first=True, enforce_sorted=False)
...
class SimpleVideoEmbeddingModule(nn.Module):
    ...
    def forward(self, vid_feats, vid_masks):
        ...
        packed_vid = nn.utils.rnn.pack_padded_sequence(vid_feats, length.to('cpu'), batch_first=True, enforce_sorted=False)
